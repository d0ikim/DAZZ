...
## 9. 작업 출력 의무 규칙 (Non-Negotiable Workflow)

> 이 섹션의 5가지 규칙은 **모든 작업에 예외 없이** 적용된다. 하나라도 빠뜨리면 작업이 완료된 것이 아니다.

### 규칙 1 — 브랜치명 선고지 (작업 시작 전)

새 기능/버그픽스/리팩토링이 별도 브랜치가 필요한 작업이라면, **코드 작성 전에 반드시 브랜치명을 먼저 고지**한다.
사용자가 직접 브랜치를 만들고 커밋하므로, Claude는 제안만 하고 임의로 `git checkout -b`를 실행하지 않는다.

형식:
> **브랜치명**: `feature/musician-claim-api`
> **이유**: 뮤지션 Claim API는 독립 기능 단위이므로 feature 브랜치로 분리

### 규칙 2 — 초보자 눈높이 설명 (작업 전 과정)

모든 구현·설명 응답에 **백엔드를 처음 접하는 사람도 이해할 수 있는 설명**을 반드시 덧붙인다.
목표: 이 프로젝트를 완벽히 이해한 개발자로서 **기술면접 100% 통과**.

설명 필수 포함 항목:
- **왜 이 설계를 선택했는가** (다른 선택지와의 트레이드오프)
- **이 코드가 어느 레이어에 속하고, 왜 거기에 있어야 하는가** (Hexagonal 관점)
- **면접에서 받을 수 있는 예상 질문 1~2개와 모범 답변**

### 규칙 3 — 커밋 정보 선제 고지 (기능 완료 시)

기능 구현·수정이 완료되면 사용자가 요청하기 전에 **반드시 먼저** 커밋 정보를 고지한다.
(사용자는 GitHub Desktop으로 직접 커밋하므로, Claude는 텍스트만 제공)

형식:
> **커밋 제목**: `feat(musician): 뮤지션 셀프온보딩 Claim API 구현`
> **커밋 본문**:
> ```
> - MusicianCommandService.claim() 구현
> - ClaimRequest DTO + 글로벌 예외 처리 연동
> ```

### 규칙 4 — PR 정보 선제 고지 (브랜치 작업 완료 시)

브랜치 작업이 완료되면 사용자가 요청하기 전에 **반드시 먼저** PR 정보를 고지한다.

형식:
> **PR 제목**: `feat: 뮤지션 셀프온보딩 Claim API`
> **PR 본문**:
> ```
> ## Summary
> - ...
...
### 규칙 5 — PR 생성 후 Opus 코드리뷰 (PR 고지 직후 의무)

PR 정보를 고지한 직후, **반드시** `code-review` 스킬을 실행한다.
- 기본: `skill=code-review, args=high --comment`
- 심층: `skill=code-review, args=ultra --comment` (클라우드 멀티에이전트, Opus)
- `--comment` 옵션은 리뷰 결과를 GitHub PR 인라인 코멘트로 자동 등록한다
- 사용자가 명시적으로 "리뷰 건너뛰어"라고 하기 전까지 이 단계는 생략 불가다
