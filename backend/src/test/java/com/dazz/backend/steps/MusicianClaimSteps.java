...
public class MusicianClaimSteps {
    ...
    public void 뮤지션이_등록되어_있다(String stageName, String position) {
        ...
        currentUuid = r.jsonPath().getString("data.uuid");
    }

    @And("해당 뮤지션이 userId {long} 에 이미 연결되어 있다")
    public void 해당_뮤지션이_이미_연결되어_있다(long userId) {
        Response r = testAdapter.post("/api/v1/musicians/" + currentUuid + "/claim",
    ...
    public void 사용자가_claim_한다(long userId) {
        response = testAdapter.post("/api/v1/musicians/" + currentUuid + "/claim",
                Map.of("userId", userId));
    }
    ...
    public void 응답의_claimed_값은_true이다() {
        assertThat(response.jsonPath().getBoolean("data.claimed")).isTrue();
    }

    @And("응답의 에러코드는 {string} 이다")
    public void 응답의_에러코드는(String errorCode) {
        assertThat(response.jsonPath().getString("error.code")).isEqualTo(errorCode);
    }
}